pub mod bflru {
    pub struct Cache4<T> {
        contents: [T; 10], // 3 extra slots on both sides as padding - actual cache is in 3..7
    }
    ...
    impl<T: Copy> Cache4<T> {
        ...
        // In structured files, this is useful to prevent outlier offsets from
        // polluting the cache, making the more commonly used offsets more
        // expensive to code.
        pub fn insert(&mut self, value: T, index: usize) {
            self.contents[index + 6] = self.contents[index + 5];
            self.contents[index + 5] = self.contents[index + 4];
            self.contents[index + 4] = self.contents[index + 3];
            self.contents[index + 3] = value;
        }
    }
    ...
    impl<T> Cache8<T> {
        // Gets an element from the cache as specified by the `index` parameter,
        // where 0 is the most recently used element, and 7 is the least recently used.
        pub fn get(&mut self, index: usize) -> &T {
            let ix4 = index * 4;
        ...
        pub fn insert(&mut self, value: T, index: usize) {
            ...
            let mask = (1 << ix4) - 1;
            let moved = ((self.order & !mask) << 4) | (slot << ix4);
            self.order = (self.order & mask) | moved;
    ...
    impl<T> Cache16<T> {
        // Gets an element from the cache as specified by the `index` parameter,
        // where 0 is the most recently used element, and 15 is the least recently used.
        pub fn get(&mut self, index: usize) -> &T {
            let ix4 = index * 4;
            let slot = (self.order >> ix4) & 0xF;
    ...
    mod tests {
        ...
        fn cache4insert() {
            ...
            lru.insert(1, 0);
            assert_eq!(*lru.get(0), 1);

            for i in 2..4 {
                let idx = (i - 1) as usize;

                lru.insert(i, idx);
                assert_eq!(*lru.get(idx), i);
                assert_eq!(*lru.get(0), i);
                assert!(*lru.get(idx) != i);
            }
        }
